<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:29:"../template/mobile/login.html";i:1595437174;}*/ ?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="UTF-8">
    ...
    <title>登录</title>
    <link rel="stylesheet" href="/Public/mobile/css/base.css?k=28396"/>
...
<div class="mobile">
    ...
    <div style="text-align: center;">
        ...
        <div class="login_bg">
	        <form action="/mobile/login.html" method="post">
				...
				<div style="margin:5px;line-height:22px;height:22px;"> <input type="checkbox" id="save" >记住密码</div>
				<script>
					$("[name=account]").val(localStorage.getItem('account',''));
					$("[name=password]").val(localStorage.getItem('password',''));
					 
					$("#save").prop("checked",$("[name=account]").val()!='');
					
					function saveLogin(){
						if($("#save").is(':checked')){
							localStorage.setItem('account', $("[name=account]").val());
                ...
                <input type="submit" class="input_btn" value="登录" onclick="saveLogin()" />
                <p class="p2 re"><a href="reg.html">点击注册</a></p>
                <p class="p1"><a href="forget.html">忘记密码?</a></p>	        </form>
        </div>
    </div>
</body>
</html>